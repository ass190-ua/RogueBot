...
void Game::processInput() {
  ...
  // --------------------------------------------------------
  // Esto asegura que la UI cambie los iconos (Teclado vs Mando) en cualquier
  // menú.
  ...
  bool keysPressed = IsKeyDown(KEY_W) || IsKeyDown(KEY_A) || IsKeyDown(KEY_S) ||
                     IsKeyDown(KEY_D) || IsKeyDown(KEY_UP) ||
                     IsKeyDown(KEY_DOWN) || IsKeyDown(KEY_LEFT) ||
                     IsKeyDown(KEY_RIGHT) || IsKeyDown(KEY_ENTER) ||
                     IsKeyDown(KEY_SPACE) || IsKeyDown(KEY_ESCAPE);
  ...
  if (IsGamepadAvailable(gpCheat) && cheatTimer <= 0.0f) {
    ...
    if (pressedInput != -1) {
      ...
      std::cout << "[CHEAT] Detectado: " << pressedInput << " (Paso "
                << cheatCodeIndex + 1 << "/10)\n";
      ...
      if (pressedInput == konamiCode[cheatCodeIndex]) {
        ...
        cheatCodeIndex = 0;
        // Si el error fue pulsar 'Arriba', cuenta como el inicio de un nuevo
        // intento
  ...
  if (togglePause) {
    ...
    // Si estamos Pausados -> Reanudar
      ...
      // Solo reactivamos música si volvemos al juego normal (Tutorial es
      // silencio/sfx)
  ...
  if (state == GameState::Victory || state == GameState::GameOver) {
    ...
    if (IsGamepadAvailable(gpId)) {
      escPressed =
          escPressed ||
          IsGamepadButtonPressed(gpId, GAMEPAD_BUTTON_RIGHT_FACE_RIGHT) ||
  ...
  if (state != GameState::MainMenu && state != GameState::OptionsMenu &&
      state != GameState::Paused && state != GameState::Tutorial &&
...
void Game::handleMenuInput() {
  ...
  if (showHelp) {
    ...
    Rectangle backHit = {(float)(tx - 6), (float)(ty - 4), (float)(tw + 12),
                         (float)(backFs + 8)};
    ...
    if (IsGamepadAvailable(menuGamepad)) {
      gpBack =
          IsGamepadButtonPressed(menuGamepad,
  ...
  Rectangle playBtn = {(float)((screenW - bw) / 2), (float)startY, (float)bw,
                       (float)bh};
  Rectangle readBtn = {(float)((screenW - bw) / 2), (float)(startY + bh + gap),
                       (float)bw, (float)bh};
  ...
  Rectangle settingsRect = {(float)(screenW - settingsSize - 20),
                            (float)(screenH - settingsSize - 20),
                            (float)settingsSize, (float)settingsSize};

  // El rectángulo "diffRect" solo es relevante si showSettingsMenu es true,
  // pero aquí estamos en el menú principal para ir a opciones, así que
  // comprobamos el botón "settingsRect".
...
void Game::handlePauseInput() {
  ...
  Rectangle btnResume = {(float)centerX - btnW / 2, (float)startY, (float)btnW,
                         (float)btnH};
  Rectangle btnSettings = {(float)centerX - btnW / 2,
                           (float)startY + btnH + gap, (float)btnW,
                           (float)btnH};
  Rectangle btnExit = {(float)centerX - btnW / 2,
                       (float)startY + (btnH + gap) * 2, (float)btnW,
                       (float)btnH};
...
void Game::handleOptionsInput() {
  ...
  if (showDifficultyWarning) {
    ...
    if (confirm) {
      ...
      std::cout << "[GAME] Dificultad actualizada a " << (int)difficulty
                << ". Run reiniciada.\n";
    } else if (cancel) {
      showDifficultyWarning = false;
    }
  ...
  bool enter = IsKeyPressed(KEY_ENTER) || IsKeyPressed(KEY_SPACE) ||
               (IsGamepadAvailable(gpId) &&
                IsGamepadButtonPressed(gpId, GAMEPAD_BUTTON_RIGHT_FACE_DOWN));
  ...
  Rectangle diffRect = {(float)(centerX - btnW / 2), (float)(screenH / 3),
                        (float)btnW, (float)btnH};
  Rectangle langRect = {(float)(centerX - btnW / 2),
                        diffRect.y + diffRect.height +
                            btnH * 0.25f, // separación visual
                        (float)btnW, (float)btnH};
  Rectangle backRect = {(float)(centerX - btnW / 2),
                        (float)(screenH - screenH / 4), (float)btnW,
                        (float)btnH};
  ...
  // Definimos un área de colisión más alta (40px) para que sea fácil "agarrar"
  // la barra
...
void Game::handlePlayingInput(float dt) {
  ...
  if (IsKeyDown(KEY_W) || IsKeyDown(KEY_A) || IsKeyDown(KEY_S) ||
      ...
      IsKeyDown(KEY_LEFT) || IsKeyDown(KEY_RIGHT) || IsKeyDown(KEY_E) ||
      IsKeyDown(KEY_I) || IsKeyDown(KEY_O) || IsKeyDown(KEY_SPACE) ||
      IsKeyDown(KEY_ONE) || IsKeyDown(KEY_TWO) ||
  ...
  if (IsGamepadAvailable(gpId)) {
    bool gpActive = false;
    // Check botones
    for (int b = GAMEPAD_BUTTON_UNKNOWN + 1; b <= GAMEPAD_BUTTON_RIGHT_THUMB;
         b++) {
  ...
  if (moveMode == MovementMode::StepByStep) {
    ...
    moveCooldown -= dt;
    bool pressedNow =
        IsKeyPressed(KEY_W) || IsKeyPressed(KEY_S) || IsKeyPressed(KEY_A) ||
        IsKeyPressed(KEY_D) || IsKeyPressed(KEY_UP) || IsKeyPressed(KEY_DOWN) ||
        IsKeyPressed(KEY_LEFT) || IsKeyPressed(KEY_RIGHT) || gpDpadPressed;
  ...
  if (IsGamepadAvailable(gpId) &&
      IsGamepadButtonPressed(gpId, GAMEPAD_BUTTON_RIGHT_FACE_LEFT))
    attackHands = true;
  ...
  if (IsGamepadAvailable(gpId) &&
      IsGamepadButtonPressed(gpId, GAMEPAD_BUTTON_RIGHT_TRIGGER_1))
    attackSword = true;

...
// Aquí es donde realmente reservamos la memoria para esa variable global.
AttackRuntime gAttack{};

// Matemáticas vectoriales
// Convierte un vector libre (ej: input analógico) en una dirección cardinal estricta.
// Compara las magnitudes absolutas de X e Y para ver cuál eje "gana".
...
std::vector<IVec2> computeMeleeTiles(IVec2 center, IVec2 lastDir, int range, bool frontOnly) {
    ...
    // Reservamos memoria anticipadamente para evitar re-allocations (optimización)
...
std::vector<IVec2> computeMeleeTilesOccluded(IVec2 center, IVec2 lastDir, int range, bool frontOnly, const Map& map) {
    ...
    auto pushRay = [&](IVec2 dir) {
        for (int t = 1; t <= range; ++t) {
            ...
            // El 'break' impide que se añadan las casillas que están detrás de la pared.
            if (!walkable(tx, ty))
                break; 

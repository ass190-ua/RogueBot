...
// -1 si v < 0
//  0 si v == 0
// Fundamental para convertir una distancia (ej: -5 tiles) en un paso de
// movimiento (-1 tile).
...
bool Enemy::inChaseRange(int px, int py, int tileSize, int radiusPx) const {
  ...
  // Multiplicamos por tileSize para convertir la distancia de "baldosas" a
  // "píxeles".
...
void Enemy::stepChase(int px, int py, const Map &map) {
  ...
  // Si ya estamos en la misma casilla, no hacer nada (evita división por cero o
  // lógica extra)
  if (dx == 0 && dy == 0)
    return;

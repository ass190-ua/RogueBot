...
public:
  ...
  // Dibuja la interfaz principal: Barra de vida, minimapa, cooldowns, texto de
  // ayuda.
...
private:
  ...
  // 'const'. Necesario porque 'drawPlaying' es const (no cambia el juego), pero
  // el HUD necesita recordar cuánta vida había en el frame anterior para
  // detectar si debe disparar una animación de daño o curación.
  mutable int prevHp = -1;

  // Estructura interna para gestionar efectos visuales temporales en la barra
  // de vida.
  struct HpFx {
    enum class Type {
      Lose,
      Gain
    }; // Tipo: Perder corazón (explosión) o ganar (crecer)
    Type type;
  ...
  // Dibuja una pequeña explosión de partículas (usada cuando se rompe un
  // corazón).
